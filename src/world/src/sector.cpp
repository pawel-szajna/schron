...
void Sector::recalculateBounds()
{
    if (not walls.empty())
    {
        boundsLeft = std::min_element(walls.begin(),
                                      walls.end(),
                                      [](const auto& a, const auto& b) { return a.xStart < b.xStart; })
                         ->xStart;
        boundsRight =
            std::max_element(walls.begin(), walls.end(), [](const auto& a, const auto& b) { return a.xEnd < b.xEnd; })
                ->xEnd;
        boundsTop = std::min_element(walls.begin(),
                                     walls.end(),
                                     [](const auto& a, const auto& b) { return a.yStart < b.yStart; })
                        ->yStart;
        boundsBottom =
            std::max_element(walls.begin(), walls.end(), [](const auto& a, const auto& b) { return a.yEnd < b.yEnd; })
                ->yEnd;
    }
}
...
const std::string& Sprite::texture(double angle) const
{
    ...
    auto it = std::find_if(textures.begin(), textures.end(), [angle](const auto& t) { return t.angle > angle; });
    return (--it)->texture;
}
} // namespace world
